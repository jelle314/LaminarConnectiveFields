function data = ccmPlotLatSigma(vw, rm, pf, binsize)
...
% gather targetROI referred pRF laterality
lat = rmGet(rm.model{1},'latx'); 
lat = lat(roiInd);
lat(lat > 0.5) = 1 - lat(lat > 0.5);
...
% plotting parameters
xaxislim = [thresh.lat(1) thresh.lat(2)];
MarkerSize = 8;

% find useful data given thresholds
ii = varexp > thresh.varexp & lat >= thresh.lat(1) & lat <= thresh.lat(2) & sig > 0.0001; 
if ~any(ii), ii = ecc > thresh2.ecc(1) & ecc < thresh2.ecc(2) & varexp > thresh.ve; end

% weighted linear regression:
p = linreg(lat(ii),sig(ii),varexp(ii));
p = flipud(p(:)); 
xfit = thresh.lat;
yfit = polyval(p,xfit);
...
data.xfit = xfit(:);
data.yfit = yfit(:);
data.x    = (thresh.lat(1):binsize:thresh.lat(2))';
...
if exist('bootstrp','file')
    B = bootstrp(1000,@(x) localfit(x,lat(ii),sig(ii),varexp(ii)),(1:numel(lat(ii))));
    B = B';
    pct1 = 100*0.05/2;
    pct2 = 100-pct1;
...
% plot averaged data
for b=thresh.lat(1):binsize:thresh.lat(2),
    bii = lat >  b-binsize./2 & ...
          lat <= b+binsize./2 & ii;
    if any(bii),
...
if pf
    ...
    ylabel('Connective Field Radius (mm)');xlabel('pRF laterality index');
