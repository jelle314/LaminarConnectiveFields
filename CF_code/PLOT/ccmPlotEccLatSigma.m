function data = ccmPlotEccLatSigma(vw, rm, pf, binsize, range)
...
% we now correct for laterality
tmp = linreg(lat(ii),sig(ii),varexp(ii));
correction = (2.*lat(ii)).*tmp(1);
sig(ii) = sig(ii) + correction;
...
if pf
    ...
    ylabel('Sampling Radius (mm)'); xlabel('Eccentricity (deg)');
